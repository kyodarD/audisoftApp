        ...
        <div class="user-panel mt-2 pb-3 d-flex">
            <div class="image">
                ...
                @if (!empty(Auth::user()->photo) && file_exists(public_path($userPhotoPath)))
                    <img class="img-circle elevation-2" src="{{ secure_asset($userPhotoPath) }}" alt="{{ Auth::user()->name }}">
                @else
                    <img src="{{ secure_asset('backend/dist/img/user2-160x160.jpg') }}" class="img-circle elevation-2" alt="">
                @endif
            </div>
